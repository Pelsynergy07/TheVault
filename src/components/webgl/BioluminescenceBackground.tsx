...
export function BioluminescenceBackground() {
  ...
  useEffect(() => {
    ...
    const gl = canvas.getContext('webgl', { alpha: true, antialias: false, preserveDrawingBuffer: false, powerPreference: 'high-performance' });
    if (!gl) return;
    ...
    const dpr = isIntegrated ? 0.5 : Math.min(window.devicePixelRatio || 1, 2);
    ...
    const uAmpDecay = gl.getUniformLocation(prog, 'u_ampDecay');
    const uMouse = gl.getUniformLocation(prog, 'u_mouse');

    const timeScaleVal = 0.15;
    const ampDecayVal = 0.48;
